...
- (NSData *)sendSynchronousRequest:(NSURLRequest *)request returningResponse:(NSHTTPURLResponse * _Nullable *)response error:(NSError * _Nullable *)error retryCount:(NSInteger)retryCount {
    ...
    if (!callResponse.statusCode || NSLocationInRange(callResponse.statusCode, NSMakeRange(500, 599))) {
        if (retryCount < 5) {
            ...
            __strong id<AloomaDelegate> strongDelegate = self.delegate;
            if (strongDelegate) {
                NSError *error = [NSError errorWithDomain:@"Alooma" code:0 userInfo:@{NSLocalizedDescriptionKey: @"Alooma tried 5 times to report unsucessfully.", @"response": callResponse, @"error": callError}];
